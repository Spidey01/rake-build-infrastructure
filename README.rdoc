...
Settings for building your project are saved in Rk/conf/cpu.os.toolset.yml and will be used in conjunction with Build/cpu/os/toolset and Dist/cpu/os/toolset. In addition if it exists, Source/module/conf/cpu.os.toolset.yml will be loaded to override the global settings on a per-module basis.
...
When Rk/conf/cpu.os.toolset.yml and the optional Source/Module/conf/cpu.os.toolset.yml file are parsed, the Builder object configures itself using the supplied 'language code' as an index into the YAML configuration.
...
Each top level key under programs, options, and template_vars sections can be accessed as ${KEY NAME}. Namely the cxxflags option becomes ${CXXFLAGS}, and so on. Any occurence of the variable in a command line expression, will be expanded before execution.
...
The template_vars section is a solution to an irksome problem. Here is the rational behind it: Unix compilers expect arguments like -lname and some compilers (msvc) expect name.lib. For portability sake, we keep constructos like shlib_file simple (compiler agnostic), and implementing something like a GccBuilder and MsvcBuilder class are the **wrong** solutions.
