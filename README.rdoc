...
The Build and Dist directories are sub divided into a hierarchy of cpu/os directories. Where CPU and OS are the names reported by Platform::ARCH and Platform::OS. This is done so that files created during the build phase, and for distribution to end users, can exist side by side on a per CPU and Operating System combination.
...
Settings for building your project are saved in Rk/conf/cpu.os.yml and will be used in conjunction with Build/cpu/os/ and Dist/cpu/os. In addition if it exists, Source/module/conf/cpu.os.yml will be loaded to override the global settings on a per-module basis.
...
Which will import classes named Paths, Builder, and VendorBuilder. Other builder classes should follow the same convention of supplying MyBuilder and MyVendorBuilder classes.
...
The VendorBuilder class is a simple extension of Builder, that understands how to use Vendor/ and Build/cpu/os/Vendor/ in place of Source/ and Build/cpu/os/.



The builder class doesn't know much about rake, so it can be stripped down and used as a general purpose software builder independent of rake. Most of the rake specific elements of Builder, are provided for convience with rake, and are easily factored out if you wish to hack Builder into something else.
...
Take a look at the Examples/ directory for a small but detailed example of what a project  will look like.
...
Example:
	...
	=> 'Source/Cheese/Foo.c'
	b.builddir 'Foo.obj'
	=> 'Build/x86/win32/Cheese/Foo.obj'
	b.distdir 'Foo.exe'
	=> 'Dist/x86/win32/Foo.exe'
...
example:
	...
	b.make_library(b.distdir('Cheese.dll'), objects)

Will compile the files Foo.c and Bar.c into object files, and then create a library from them.


Because not every programming language works the same way, these methods may not be sufficiant to build any project. For just this reason Builder can be subclassed to add aditional behaviour. In this particular example, since we are using the C programming language, we should prefer using the CBuilder class. CBuilder ads make_static_library and make_shared_library methods, among other things helpful for C like projects.


==== Task Creation

To make life with rake easier, you can create directory/file tasks through Builder. As a convience method, Builder provides an obj_file method that is anoglogus to rakes file method.
...
When Rk/conf/cpu.os.yml and the optional Source/Module/conf/cpu.os.yml file are parsed, the Builder object configures itself using the supplied 'language code' as an index into the YAML configuration.
...
In the options section, we define a nested set of mappings that define the arguments for the entries in the programs section. How this is done, is by defining a ${program name}flags mapping. What ${program name} is, is in fact irrelevent. I decided to just stick with the CFLAGS/CXXFLAGS convention that I'm accustomed to 8=). You could just as easily call it foobar instead.


Each member of the ${program name}flags mapping is expected to be an abritary set of key: value pairs. This was done to allow adding a construct like ${CXXFLAGS:WARNINGS} at a later date. All of the fields of ${program name}flags will be joined and made available in the commands section under a name like ${CFLAGS} where ${program name} is c.
...
Which creates the variables ${ARFLAGS}, ${CFLAGS}, ${CXXFLAGS}, and ${LDFLAGS}. The various flags names were chosen to align with common conventiion. The sub keys were chosen for discriptive value.
...
Because this is all designed to be used with rake, and different implementations of C/C++ can differ in details, like whether it is objectfile.o or objectfile.obj, a section like this was neccessary for convenience
...
Which will equip CBuilder with the neccessary extension infomation
...
Each top level key under programs and options can be accessed as ${KEY NAME}. Namely the cxxflags option becomes ${CXXFLAGS}, and so on. Any occurence of the variable in a command line expression, will be expanded before execution.
...
Example:
	...
	commands:
		...
		make_executable:
		    ${CXX} ${CXXFLAGS} /Fe"${TARGET}" ${SOURCE}
		make_library:
		    ${CXXFLAGS} ${CXXFLAGS} /LD /Fe"${TARGET}" ${SOURCE} /link ${LDFLAGS}

Would provide suitable instructions for Builder to compile objects using Visual C++. Again, in this case the CBuilder class should have been prefered.
...
When applicable, name your classes {Language Name}Builder and {Language Name}VendorBuilder, and save them in Rk\{language name}builder.rb.
